...
function ToolbarButton({ icon: Icon, title, onClick }: { icon: React.FC<{ className?: string }>; title: string; onClick: () => void }) {
  return (
...
const MathIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
    <path d="M4 20h4l4-16h4" /><path d="M15 7h5" /><path d="M17.5 4.5v5" />
  </svg>
);
...
export default function AdminEditor() {
  ...
  const insertMarkdown = useCallback((before: string, after: string = '', defaultText: string = '', blockLevel: boolean = false) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => { if (dirtyRef.current) handleSave(); }, 30000);
  ...
  if (loading) {
    return <div className="p-8 flex justify-center"><div className="w-6 h-6 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" /></div>;
  }
  ...
  return (
    <div className="flex flex-col h-full -mx-6 -my-8">
      <div className="border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-4 lg:px-6">
        <div className="flex items-center justify-between h-14">
          <div className="flex items-center gap-3 min-w-0">
            <Link href="/admin/files" className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"><ArrowLeft className="w-4 h-4" /></Link>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-md">
              <button onClick={() => setViewMode('edit')} className={`p-2 rounded-l-md transition-colors ${viewMode === 'edit' ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-400'}`} title="Edit"><Edit className="w-4 h-4" /></button>
              <button onClick={() => setViewMode('split')} className={`p-2 border-x border-gray-200 dark:border-gray-700 transition-colors hidden lg:block ${viewMode === 'split' ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-400'}`} title="Split"><Columns className="w-4 h-4" /></button>
              <button onClick={() => setViewMode('preview')} className={`p-2 rounded-r-md transition-colors ${viewMode === 'preview' ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-400'}`} title="Preview"><Eye className="w-4 h-4" /></button>
            </div>
            <button onClick={handleSave} disabled={saving} className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50">
      ...
      <div className={`flex-1 ${viewMode === 'split' ? 'flex' : ''}`}>
        {showEditor && (
          <textarea ref={textareaRef} value={markdown} onChange={e => { setMarkdown(e.target.value); setDirty(true); }}
          ...
          <div className={`${viewMode === 'split' ? 'w-1/2' : 'w-full'} overflow-y-auto`}>
            <div className="max-w-3xl mx-auto px-6 py-8">
              <article className="prose prose-gray dark:prose-invert max-w-none">
                <ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex, rehypeHighlight]}>{markdown}</ReactMarkdown>
              </article>
            </div>
          </div>
        )}
